    ...
    if (this.currentPage === 'editor' && pageName !== 'editor') {
      if (window.editorApp && window.editorApp.hasUnsavedChanges()) {
        const confirmLeave = confirm(window.languageManager.currentLanguage === 'th' ? 
          'คุณมีการเปลี่ยนแปลงที่ยังไม่ได้บันทึก หากคุณออกจากหน้านี้ การเปลี่ยนแปลงทั้งหมดจะหายไป' : 
...
class EditorApp {
  ...
  // ตรวจสอบ THREE.js และ GLTFLoader
  checkDependencies() {
    ...
    if (!hasDRACOLoader) {
      console.warn('⚠️ DRACOLoader is not loaded - compressed models may not work');
    } else {
      console.log('✅ DRACOLoader is available');
  ...
  // เพิ่มฟังก์ชันช่วยตรวจสอบโมเดลที่มีอยู่จริง
  // ฟังก์ชันตรวจสอบไฟล์โมเดลที่มีอยู่และแสดงข้อมูลดีบัก
  logAvailableModels() {
    console.group('🔍 3D Model Availability Check');
    ...
    // แสดงโครงสร้างของ this.furnitureData ถ้ามี
    if (this.furnitureData) {
      console.log('📋 Furniture data configuration:', this.furnitureData);
      
      // แสดงรายการพาธของโมเดลทั้งหมดจาก furnitureData
      console.log('📁 Model paths from furniture data:');
      ...
      console.warn('⚠️ Furniture data not available yet');
    }
    
    console.log('🧪 Testing model availability...');
    
    // ลองทดสอบโหลดไฟล์โดยใช้ fetch เพื่อตรวจสอบว่าไฟล์มีอยู่จริงหรือไม่
    ...
    Promise.all(fetchPromises).then(results => {
      ...
      console.log(`\n📊 Results: ${available.length} found, ${missing.length} missing`);
      
      if (missing.length > 0) {
        console.warn('⚠️ Missing models may cause fallback boxes to appear instead of 3D models');
  ...
  showFurnitureCategory(category) {
    ...
    categoryData.forEach(item => {
      ...
      // ตรวจสอบว่ามี 3D model preview หรือไม่
      if (item.preview && (item.preview.endsWith('.glb') || item.preview.endsWith('.gltf'))) {
        // สร้าง canvas สำหรับแสดง 3D preview
      ...
      // ถ้าเป็น 3D model preview ให้โหลดและแสดงโมเดล
      if (item.preview && (item.preview.endsWith('.glb') || item.preview.endsWith('.gltf'))) {
        const canvasId = `preview-${item.id}-${Date.now()}`;
        ...
        // ถ้า element พร้อมแล้ว ให้โหลดทันทีหลังจากเพิ่มเข้า DOM
        setTimeout(() => {
    ...
    return new Promise((resolve, reject) => {
      ...
      if (!GLTFLoaderClass) {
        ...
        if (typeof THREE.GLTFLoader === 'undefined' && typeof window.GLTFLoader !== 'undefined') {
          ...
          console.error('GLTFLoader ไม่พบ กรุณาเพิ่ม script ใน HTML:');
      ...
      // ตรวจสอบและใช้งาน DRACOLoader เพื่อบีบอัดโมเดล
  ...
  // ฟังก์ชันสำหรับแสดง 3D preview ของเฟอร์นิเจอร์ในแถบด้านข้าง
  render3DPreview(canvasId, item) {
    ...
    // สร้าง Three.js scene สำหรับ preview
    ...
    if (GLTFLoaderClass) {
      ...
      loader.load(adjustedPath, (gltf) => {
        ...
        // ถ้าโหลดล้มเหลว ให้แสดงข้อมูลเฟอร์นิเจอร์ทั่วไป
  ...
  addFurnitureToScene(furnitureItem) {
    ...
    // ทดสอบโหลด THREE และ GLTFLoader
    this.checkDependencies();
    ...
    if (furnitureItem.hasModel && furnitureItem.modelPath) {
      ...
      // ตรวจสอบว่าไฟล์โมเดลมีอยู่หรือไม่ และแก้ไข path ให้ถูกต้อง
      // path จำเป็นต้องเป็นแบบ relative จากรากของเว็บไซต์
      ...
      // แสดงรายการโมเดลที่มีอยู่ในโปรเจค (เพื่อช่วยแก้ไขปัญหา)
      ...
      this.loadGLTFModel(adjustedPath) // ใช้ adjustedPath ที่แก้ไขแล้ว
        ...
        .catch(error => {
          ...
          console.warn('1. ไฟล์โมเดลไม่มีอยู่ที่ path ที่ระบุ');
          console.warn('2. การอ้างอิง path ไม่ถูกต้อง (ควรใช้ path แบบ relative เช่น models/furniture/chair.glb)');
          console.warn('3. GLTFLoader หรือ DRACOLoader ไม่ถูกโหลดอย่างถูกต้อง');
          console.warn('4. โมเดลอาจเสียหายหรือมีรูปแบบไม่ถูกต้อง');
          
          const message = window.languageManager?.currentLanguage === 'en'
            ? 'Could not load 3D model. Showing box instead.'
            : 'ไม่สามารถโหลดโมเดล 3D ได้ จะแสดงเป็นกล่องแทน';
            
          this.showFeedback(message, 'viewport-help', 5000); // แสดงข้อความนานขึ้นเป็น 5 วินาที
          
          // สร้างกล่องแทนโมเดลที่โหลดไม่สำเร็จ
          ...
          box.userData = { 
            ...
            selected: false,
            outline: null,
            isFallback: true // เป็นโมเดลสำรอง
          };
          
          // เพิ่มกล่องลงในฉาก
          this.scene.add(box);
          
          // ทำให้กล่องสามารถโต้ตอบได้
          this.makeInteractive(box);
          
          // บันทึกสถานะเพื่อ undo
          this.saveState();
          
          return box; // ส่งคืนกล่องแทนโมเดล
  ...
  createBoxModel(furnitureItem) {
    ...
    const material = new THREE.MeshLambertMaterial({ 
      ...
      transparent: true,
      opacity: 0.8,
      wireframe: false // ตั้งเป็น true ถ้าต้องการให้เห็นเป็นโครงลวด
    });
    ...
    return {
      bedroom: [
        { 
          ...
          nameEn: 'Bed',
          type: 'bed',
          modelPath: 'models/furniture/bed.glb', // ไม่มี / นำหน้า - path ถูกต้องแล้ว
          hasModel: true, // มีโมเดลจริง
          preview: 'images/furniture/bed.glb', // ใช้โมเดลเดียวกันเป็น preview
          color: '#8B4513', 
        ...
        { 
          ...
          nameEn: 'Desk',
          type: 'desk',
          modelPath: 'models/furniture/desk.glb', // แก้ไขให้ไม่มี / นำหน้า
          hasModel: false, // ยังไม่มีโมเดล
          preview: 'images/furniture/desk_preview.jpg', // แก้ไข path ให้ถูกต้อง
          color: '#DEB887', 
        ...
        { 
          ...
          nameEn: 'Chair',
          type: 'chair',
          modelPath: 'models/furniture/chair.glb', // path ถูกต้องแล้ว
          hasModel: true, // มีโมเดล 3D
          preview: 'models/furniture/chair.glb', // ใช้โมเดล 3D เป็น preview
          color: '#654321', 
